import 'package:flutter/material.dart'; // wajib untuk Color
...
class BidangControllerHttp extends GetxController {
  ...
  Future<void> fetchBidang({String? testMode}) async {
    ...
    if (testMode == '404') endpoint = '$base/salah';
    if (testMode == 'badurl') endpoint = 'https://domain-tidak-ada.com';
    if (testMode == 'timeout') endpoint = 'https://10.255.255.1';

    try {
      final res =
          await http.get(Uri.parse(endpoint)).timeout(const Duration(seconds: 5));

      if (res.statusCode == 200) {
        final List data = jsonDecode(res.body);
        bidangList.assignAll(
          data.map((e) => BidangModel.fromMap(e)).toList(),
        );
